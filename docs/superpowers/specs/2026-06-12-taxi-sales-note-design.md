...
- 日付: 2026-06-12
- ステータス: 承認済み（ユーザー承認 2026-06-12）。v1.1 で機能統合を追記。
- 種別: 新規・個人完結アプリ（静的PWA）

> **v1.1 追記（2026-06-12）**: taxicrew-portal の乗務員機能のうち**端末内で完結できるもの**を統合する。
> サーバーなし・データ端末内のみの原則は維持。詳細は「11. v1.1 機能統合」を参照。
...
## 11. v1.1 機能統合（taxicrew-portal の乗務員機能のうち端末内完結分）

承認: 2026-06-12。点呼ログは**除外**（ユーザー指示「不要」）。サーバー必須機能（チャット/会社お知らせ/正式申請）も除外。

### 11.1 追加する機能（すべて端末内・自分で入力 or 自動計算）

1. **出番・シフト**（新ストア `shifts`）
   - フィールド: `id`(uuid), `date`(YYYY-MM-DD), `kind`(出番|明休|公休|有休|その他), `planStart`/`planEnd`(HH:MM|null), `memo`, `createdAt`/`updatedAt`
   - 月リスト表示。ホームの「今日の出番」に当日 `date` の予定を表示（無ければ「予定なし」）。
2. **給与明細（自分用概算）**（`settings` 拡張）
   - 設定: `payRate`(歩合率% number, 既定 0), `baseSalary`(基本給 number, 既定 0)
   - 概算 = 売上合計 × payRate/100 + baseSalary（月別）。あくまで自分用の目安。会社の正式明細ではない旨をUIに明記。
3. **プロフィール**（`settings` 拡張）
   - `employeeNo`(社員番号), `office`(所属営業所), `carNo`(車番), `licenseNo`(免許番号) を追加。`driverName` は既存。

### 11.2 画面・ナビ

- ホームを taxicrew-portal 風に再構成: 「今日の出番」→ 売上サマリー（売上合計/営業回数/手取り概算）→ メニューカード（出番・シフト / 給与明細 / プロフィール）。
- 下部ナビは既存（ホーム/入力/履歴/出力/設定）を維持し、メニューカード経由で各機能へ遷移。
- 既存の売上入力・履歴・エクスポート・合言葉ゲート・ローカルロック・PWA は**非破壊で維持**。

### 11.3 非機能・制約

- 公開URL・合言葉・データ端末内のみ・オフラインPWAは変更なし。
- バックアップ/復元(JSON)は新ストア `shifts` と拡張 `settings` も含める。
- `_selftest.html` に給与概算計算・shift当日判定のアサーションを追加。

### 11.4 受け入れ基準（追加分）

- [ ] ホームに「今日の出番」が出る（当日shiftの有無で表示が変わる）。
- [ ] 出番・シフトを追加/編集/削除でき、月リストに反映される。
- [ ] 設定で歩合率・基本給を入れると給与概算が月別に算出される。
- [ ] プロフィール項目を保存・表示できる。
- [ ] 既存機能（売上入力/履歴/Excel・CSV出力/合言葉/ロック/PWA）が壊れていない。
- [ ] バックアップJSONに shifts と拡張settings が含まれ、復元できる。

## 12. v1.2 機能追加（写真OCR入力・給与明細記録）

承認: 2026-06-12。端末内のみ・合言葉・PWAの原則は維持。

### 12.1 写真から売上入力（端末内OCR）

- 売上入力画面に「写真から入力」ボタン（カメラ撮影/画像選択 `<input type="file" accept="image/*" capture>`）。
- 画像を canvas で長辺~1000pxにJPEG圧縮 → **Tesseract.js（端末内OCR）**で数値を読取り。
  - OCR部品（tesseract core + lang data, 約10〜15MB）は**初回使用時にオンデマンドDL→Cache Storageにキャッシュ**（アプリシェルには含めない＝本体は軽量。初回OCRのみ通信要、以後オフライン可）。
  - 数字認識は eng + 数字ホワイトリスト中心。抽出した数値候補から各項目（trips/cash/card/ic/qr/ticket 等）を**ベストエフォートでプレフィル**。
- **項目は既存の売上入力と同一**。ユーザーが同じフォームで確認・修正して保存（OCRは下書き支援）。
- 撮影画像はエントリに**任意添付**（`entries.photo` を追加。JPEG dataURL、圧縮済み）。
- 画像は外部送信しない（端末内のみ）。精度は不安定なため必ず手直し前提のUIにする。

### 12.2 給与明細の記録（数値＋写真）

- 新ストア `payslips`: id(uuid), ym(YYYY-MM), gross(支給額 number), deduction(控除 number), net(手取り number), memo, photo(dataURL|null), createdAt/updatedAt。
- 「給与明細」画面を2系統に: **概算**（既存 estimatePayroll）＋**実績（記録）**（payslips の追加/編集/削除/一覧）。
- 明細写真は任意添付（圧縮保存）。

### 12.3 データ・非機能

- `entries` に `photo`(dataURL|null) を追加（既存データは無しでも動く＝後方互換）。
- バックアップ/復元(JSON)に `payslips` と画像を含める（容量が増える旨をUIに注記）。
- sw.js: アプリシェルにOCR部品は含めない。CACHE_NAME を taxi-sales-note-v4 に更新。

### 12.4 受け入れ基準（追加分）

- [ ] 売上入力で写真を選ぶとOCRが走り、読取れた数値が項目にプレフィルされ、手修正して保存できる。
- [ ] OCR部品は初回のみDLされ、2回目以降はキャッシュから動く。
- [ ] 撮影画像をエントリに添付保存・表示できる。
- [ ] 給与明細「実績」を数値＋写真で追加/編集/削除でき、一覧表示される。
- [ ] 既存機能（売上/出番/概算給与/プロフィール/履歴/出力/合言葉/ロック/PWA）が壊れていない。
- [ ] バックアップJSONに payslips と画像が含まれ、復元できる。
