...
export async function replaceAll({ entries = [], settings = null, shifts = [] }) {
  const db = await openDb();
  const tx = db.transaction(["entries", "settings", "shifts"], "readwrite");
...
export async function mergeBackup({ entries = [], settings = null, shifts = [] }) {
  const [current, currentShifts] = await Promise.all([getAllEntries(), getAllShifts()]);
...
export async function exportAllData(includePassword = false) {
  const [entries, settings, shifts] = await Promise.all([getAllEntries(), getSettings(), getAllShifts()]);
